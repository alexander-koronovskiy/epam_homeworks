...
def checkbadchars(url):
    ...
    if proto != "file":
        ...
        if _check_for_bad_chars(host, ALLOWED_CHARS_HOST) or _check_for_bad_chars(
            uripath, ALLOWED_CHARS
        ):
...
class SvnWCCommandPath(common.PathBase):
    ...
    def _svn(self, cmd, *args):
        ...
        try:
            ...
            if strerr.find("not found") != -1:
                ...
                raise py.error.ENOENT(self)
            if (
                strerr.find("file exists") != -1
                or strerr.find("file already exists") != -1
                or strerr.find("w150002:") != -1
    ...
    def blame(self):
        ...
        for i, (blameline, line) in enumerate(zip(blamelines, reallines)):
            ...
            if not m:
                raise ValueError(
                    "output line %r of svn blame does not match "
    ...
    def info(self, usecache=1):
        ...
        if not info:
            ...
            if (
                output.strip() == ""
                or output.lower().find("not a versioned resource") != -1
            ):
    ...
    def fromstring(data, rootwcpath, rev=None, modrev=None, author=None):
        ...
        for line in data.split("\n"):
            ...
            if c0 in "?XI":
                ...
                if c0 == "?":
                    ...
                    wcpath = rootwcpath.__class__(
                        rootwcpath.localpath.join(fn, abs=1), auth=rootwcpath.auth
                    )
            ...
            if c0 == "M":
                assert wcpath.check(
                    file=1
                ), "didn't expect a directory with changed content here"
    ...
    def fromstring(data, rootwcpath, rev=None, modrev=None, author=None):
        ...
        for entryel in doc.getElementsByTagName("entry"):
            ...
            if itemstatus == "unversioned":
                ...
                wcpath = rootwcpath.__class__(
                    rootwcpath.localpath.join(path, abs=1), auth=rootwcpath.auth
                )
                rootstatus.external.append(wcpath)
                continue
            ...
            assert itemstatus != "modified" or wcpath.check(
                file=1
