...
def periodic_update(
    distribution, for_py_version, wheel, search_dirs, app_data, do_periodic_update
):
...
def handle_auto_update(distribution, for_py_version, wheel, search_dirs, app_data):
    ...
    if u_log.needs_update:
        ...
        trigger_update(
            distribution, for_py_version, wheel, search_dirs, app_data, periodic=True
        )
...
def trigger_update(
    distribution, for_py_version, wheel, search_dirs, app_data, periodic
):
    ...
    for for_py_version, distribution_to_package in BUNDLE_SUPPORT.items():
        ...
        for distribution in distribution_to_package.keys():
            thread = Thread(
                ...
                args=(app_data, distribution, for_py_version),
            )
            thread.start()
